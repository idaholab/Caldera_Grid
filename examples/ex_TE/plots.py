...
ratio_folders = [os.path.basename(os.path.normpath(folder)) for folder in glob.glob(os.path.join(output_folder, "*"))]
...
# y_lims, y_ticks, and text_y are set based on uncontrolled profile data
...
for ratio_folder in ratio_folders:
    ...
    df_u = extract_power_profile_df(os.path.join(ratio_folder, "uncontrolled"))
    ...
    y_max = find_ymax(df_u[profile].max())    
    y_lims = (0, y_max)
    ...
    if True:
        ...
        fig.legend()
        fig.tight_layout()
        fig_file_name = ratio_folder + "_uncontrolled.png"
    ...
    df_tou = extract_power_profile_df(os.path.join(ratio_folder, "time_of_use"))
    ...
    if True:
        ...
        fig.legend()
        fig.tight_layout()
        fig_file_name = ratio_folder + "time_of_use.png"
    ...
    low_folders = ["linear", "steep_cubic", "inverse_s"]
    
    timestep_hrs = 15*50/3600
    
    for low_folder in low_folders:

        files = glob.glob(os.path.join(output_folder, ratio_folder, high_folder, low_folder, "*"))
        ...
        for file in files[:]:
            ...
            df = extract_power_profile_df(os.path.join(ratio_folder, high_folder, low_folder, scenario))
            df2 = extract_cost_profile_df(os.path.join(ratio_folder, high_folder, low_folder, scenario))
            ...
            if True:
                ...
                fig.legend()
                fig.tight_layout()
                fig_file_name = ratio_folder + "_" + high_folder + "_" + low_folder + "_" + scenario + ".png"
                plt.close(fig)
                fig.savefig( os.path.join(power_profiles_figures_folder, fig_file_name), dpi = 300)
