...
parser.add_argument(
    '-fig',
    '--figures_path',
    help="Path to the output figures \
    folder. If the path does not exist then it will be created. If the \
    argument is not present, the default outputs folder will be used.",
    required=False)
parser.add_argument(
...
if args['grid_dir'] != None:
    caldera_grid_proj_dir = args['grid_dir']
else:
...
if args["libraries"] != None:
    caldera_lib_dir = args["libraries"]
else:
    caldera_lib_dir = path_to_here
    print("Defaulting the path to the python libraries to be in the same directory as start_execution.py")
...
sys.path.insert( index+1, os.path.join( caldera_lib_dir, "./libs" ) )
...
from ES400_aux import ES400_aux
...
if __name__ == '__main__':
    ...
    if args["figures_path"] != None:
        figures_path = "{}".format(args["figures_path"])
    else:
        # The default location of the 'figures'
        # directories are to be in the same directory as 'start_execution.py'.
        figures_path = os.path.join( path_to_here,"figures" )
        print("Defaulting the figures_path/ folder to be in the same directory as start_execution.py")
    ...
    print("Figures path : {}".format(figures_path))
    ...
    io_dir.figures_dir = os.path.join( working_dir, figures_path )
    ...
    num_of_federates += 1   # Caldera_ES400
    ...
    #---------------------------
    #   ES400 Control Federate
    #---------------------------
    json_config_file_name = 'Caldera_ES400.json'
    ES400_obj = ES400_aux(io_dir, simulation_time_constraints)    
    p = Process(target=typeA_control_federate, args=(io_dir, json_config_file_name, simulation_time_constraints, ES400_obj,), name="caldera_ES400_federate")
    processes.append(p)
