...
class ES500_Aggregator_log_files:
    ...
    def __init__(self, io_dir, log_optimization_values, log_pev_energy_setpoints, log_stop_charge_cycling_decision_parameters):
        ...
        if self.log_stop_charge_cycling_decision_parameters:
            self.f_stop_CC = open(io_dir.outputs_dir + '/ES500_Stop_Charge_Cycling_Parameters.csv', 'w')
        ...
        self.f_OSS = open(io_dir.outputs_dir + '/ES500_Optimization_Solver_Status.csv', 'w')
        line = 'next_aggregator_timestep_start_time, next_aggregator_timestep_start_time'
        ...
        if self.log_optimization_values:
            if self.log_optimization_values:
                self.f_OMV = open(io_dir.outputs_dir + '/ES500_Optimization_Model_Values.csv', 'w')
                ...
                self.f_E_residual = open(io_dir.outputs_dir + '/ES500_Residual_Energy.csv', 'w')
        ...
        if self.log_pev_energy_setpoints:
            if self.log_pev_energy_setpoints:
                self.f_e_step = open(io_dir.outputs_dir + '/ES500_PEV_Energy_Setpoints.csv', 'w')
