...
ratio = 0.01 # ratio of charge events to filter. 1 means all charge events are included and 0 means no charge events are included
...
def write_input_files(folder : str, cost_function : str, scenarios):
    ...
    for i, (forecast, actual) in enumerate(scenarios):
        ...
        input_folder = os.path.join(folder, cost_function, "scenario_{}".format(i))
        output_folder = os.path.join("..", "outputs", input_folder)
        
        shutil.rmtree(input_folder, ignore_errors=True)
        shutil.copytree("uncontrolled", input_folder)
        ...
        final_CE_df.to_csv(os.path.join(input_folder, "CE_controlled.csv"), index = False)
        final_SE_df.to_csv(os.path.join(input_folder, "SE_controlled.csv"), index = False)
        ...
        forecast_df.to_csv(os.path.join(input_folder, "TE_inputs", "forecast.csv"), index = False)
        ...
        actual_df.to_csv(os.path.join(input_folder, "TE_inputs", "actual.csv"), index = False)
        
        with open(os.path.join(input_folder, "TE_inputs", "generation_cost.json"), 'w') as fp:
            json.dump(generation_cost, fp, indent=4)

# Uncontrolled
clean_input_folder("uncontrolled")
output_folder = os.path.join("..", "outputs", "uncontrolled")
shutil.rmtree(output_folder, ignore_errors=True)
os.makedirs(output_folder)

final_CE_df.to_csv(os.path.join("uncontrolled", "CE_uncontrolled.csv"), index = False)
final_SE_df.to_csv(os.path.join("uncontrolled", "SE_uncontrolled.csv"), index = False)

# Activate Control Strategy
final_CE_df["Ext_strategy"] = "ext0001"

# Good Forecast
write_input_files("good_forecast", "linear", good_forecast_scenarios)
write_input_files("good_forecast", "steep_cubic", good_forecast_scenarios)
write_input_files("good_forecast", "inverse_s", good_forecast_scenarios)

# Bad Forecast
write_input_files("bad_forecast", "linear", bad_forecast_scenarios)
write_input_files("bad_forecast", "steep_cubic", bad_forecast_scenarios)
write_input_files("bad_forecast", "inverse_s", bad_forecast_scenarios)